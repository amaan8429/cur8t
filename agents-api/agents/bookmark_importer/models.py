...
class BookmarkUploadRequest(BaseModel):
    ...
    original_filename: str = Field(
        ..., description="Original filename of the bookmark file"
    )
    browser_type: Optional[str] = Field(
        None, description="Browser type (chrome, firefox, safari, edge)"
    )
    user_preferences: Optional[Dict[str, Any]] = Field(
        None, description="User categorization preferences"
    )
...
class BookmarkAnalysisRequest(BaseModel):
    ...
    max_categories: Optional[int] = Field(
        5, ge=1, le=15, description="Maximum number of categories to create"
    )
    min_bookmarks_per_category: Optional[int] = Field(
        3, ge=1, le=20, description="Minimum bookmarks per category"
    )
    preferred_categories: Optional[List[str]] = Field(
        None, description="User-suggested category names"
    )
    merge_similar_categories: bool = Field(
        True, description="Whether to merge similar categories"
    )
...
class CollectionCreationRequest(BaseModel):
    ...
    selected_categories: List[str] = Field(
        ..., description="Category names to create as collections"
    )
    custom_category_names: Optional[Dict[str, str]] = Field(
        None, description="Custom names for categories"
    )
