...
from .models import (
    ...
    BookmarkImportStatus, BookmarkSessionStatus, CollectionCreationResponse,
    BookmarkPreviewResponse
)
from core.models import ErrorResponse
from core.utils import is_valid_url, get_domain_from_url, clean_text
...
class BookmarkImporterService:
    ...
    def _detect_browser_type(self, soup: BeautifulSoup, hint: Optional[str]) -> Optional[str]:
        ...
        if soup.find('meta', {'name': 'GENERATOR', 'content': re.compile(r'Bookmarks.*Chrome', re.I)}):
            return "chrome"
    ...
    def _parse_chrome_bookmarks(self, soup: BeautifulSoup) -> Tuple[List[BookmarkItem], Dict[str, int]]:
            ...
            for item in dl_element.find_all(['dt'], recursive=False):
                ...
                if link:
                    ...
                    if is_valid_url(url):
                        ...
                        if folder_path:
                            folder_structure[folder_path] = folder_structure.get(folder_path, 0) + 1
    ...
    def _parse_firefox_bookmarks(self, soup: BeautifulSoup) -> Tuple[List[BookmarkItem], Dict[str, int]]:
    ...
    def _parse_safari_bookmarks(self, soup: BeautifulSoup) -> Tuple[List[BookmarkItem], Dict[str, int]]:
    ...
    def _parse_generic_bookmarks(self, soup: BeautifulSoup) -> Tuple[List[BookmarkItem], Dict[str, int]]:
    ...
    async def analyze_bookmarks(
        ...
        try:
            ...
            ai_confidence = categorized_count / total_bookmarks if total_bookmarks > 0 else 0.0
...
"""
            ...
            for cat_data in parsed.get("categories", []):
                ...
                if bookmark_items:  # Only create category if it has bookmarks
                    category = BookmarkCategory(
                        ...
                        suggested_collection_name=cat_data.get("suggested_collection_name", cat_data.get("name", "Untitled"))
                    )
    ...
    def get_session_status(self, session_id: str) -> Optional[BookmarkSessionStatus]:
        ...
        return BookmarkSessionStatus(
            ...
            processed_bookmarks=total_bookmarks if session["status"] in [BookmarkImportStatus.READY, BookmarkImportStatus.COMPLETED] else 0,
